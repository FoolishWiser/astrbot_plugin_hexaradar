...
每人自动保留最近 10 条更新记录（评分/备注/年龄变更明细、来源、时间；自动评审改分额外记录**理由与证据引用**）。AI 同一次回答中的多次改分合并为一条大记录。详情页点击 🕘 按钮查看。

## LLM 自动评审改分（可选）

开启配置 `auto_review` 后，插件成为**持续评估者**：对话（按 `auto_review_trigger` 配置扫描用户消息/AI 回复）提及库内人员（或其自定义别名）时，插件会在回答结束后自动发起一次评审调用——结合该人当前评分、逐项现有理由与本次对话上下文（用户消息 + AI 回复），判断是否出现了**新的、可引用的信息**（事件/行为/表态/事实）需要调整评分。改分**静默允许**并写入更新记录，随后自动发送改分提示（箭头表格 + 加粗理由 + 证据引用，模板生成不额外消耗 LLM token）。

评审纪律（防止乱改与空转）：

- **证据必填**（`auto_review_require_evidence`，默认开）：评审必须从对话原文引用一句作为证据，无证据的改分提案被拒绝
- **冷却**（`auto_review_cooldown`，默认 30 秒）：同一人短时间内重复提及不再重复发起评审，可设 0 关闭
- **单次上限**（`auto_review_max_delta`，默认 0=不限制）：开启后每个维度单次调整幅度不超过 ±N，超出自动钳制并在提示中注明
- **触发范围**（`auto_review_trigger`）：`both`=用户消息或回复提及即触发（默认）；`user`=仅用户消息提及；`reply`=仅回复提及
- **评审提供商**（`auto_review_provider`）：默认留空=跟随当前会话使用的 LLM；可从 AstrBot 已配置的提供商/模型中选择一个固定用于评审（设置页下拉），找不到时自动回退
- **不重复调分**：已反映在现有评分理由中的旧信息不再调整
- **多人同回复**：一次评审可输出多个人员的改分提案（`updates`），逐一校验后全部应用
- **可审计**：每次改分记录写入更新历史（含理由与证据引用），详情页 🕘 面板可查

评审只调整**已存在**人员，不负责新建（新建请使用 `set_radar_scores` 工具）；评审调用不启用工具、不经过回答钩子，无递归风险。
